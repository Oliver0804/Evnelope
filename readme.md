...
- ✍️ **手動輸入**：直接在頁面填寫寄件人 / 收件人資訊，可新增、編輯、複製、刪除清單項目。
- 👤 **預設寄件人**：設定一次寄件人後，新增收件人時自動帶入，並可一鍵套用到現有所有信封。
- 📥 **CSV 匯入 / 匯出**：支援點選或拖曳上傳（相容新舊格式），亦可將目前清單匯出成 CSV 備份。
- 📋 **批次貼上**：一次貼上多行收件人地址快速新增（寄件人自動套用預設值）。
- ✅ **清單快捷**：全選 / 反選、複製一筆，搭配選擇性列印更省事。
- 🎯 **列印對位校正**：X / Y 位移（mm）補償印表機進紙偏差，只影響列印不影響預覽。
- 🟥 **郵遞區號紅框格**：一鍵切換標準信封的紅色框格樣式。
...
- ✅ **選擇性列印**：可勾選清單項目只列印選取的信封，或用每筆的「列印」鈕單獨列印，不必每次都印全部。
- 🎨 **即時預覽**：所有設定即時反映，預覽自動縮放以符合畫面。
- 🖱️ **拖曳排版**：可直接在預覽上拖曳「寄件人 / 收件人 / 郵遞區號」區塊調整位置（套用到所有信封並記憶於本機），滑鼠滾輪可縮放區塊文字。
- 🔍 **預覽縮放**：滑桿 / ＋－ 按鈕 / Ctrl＋滾輪可縮放整個預覽，另有「符合畫面」與「重設排版」。
- 📌 **浮動列印鈕**：捲動頁面時左上角出現浮動列印按鈕，隨時一鍵列印。
